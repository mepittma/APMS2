...
# # # # # # # # # # CREATE EDGE FILE # # # # # # # # # # 
case_LoF_genes = unique(LoF_cases$Gene)
ctrl_LoF_genes = unique(LoF_ctrls$Gene)
...
write.csv(degree_frame, 
          file = paste0(base_path, "/intermediate/network_permutations/LoF_", today, ".csv"),
...
write.table(paste0("Network connectivity p-value: ", pval), file = paste0(out_path, "/network_pval_LoF.txt"))
...
write.csv(nodes_adj, file = paste0(out_path, "/Cytoscape_LoF_nodes.csv"), 
          row.names = FALSE, quote = FALSE)
