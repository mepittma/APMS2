...
# # # # # # # # # # CREATE EDGE FILE # # # # # # # # # # 
case_DNV_genes = unique(DNV_cases$Gene)
ctrl_DNV_genes = unique(DNV_ctrls$Gene)
syn_DNV_genes = unique(DNV_case_syn$Gene)
iRef_cases = unique(PPI[which(PPI$aliasA %in% case_DNV_genes & PPI$aliasB %in% case_DNV_genes),])
iRef_ctrls = unique(PPI[which(PPI$aliasA %in% ctrl_DNV_genes & PPI$aliasB %in% ctrl_DNV_genes),])
iRef_syn = unique(PPI[which(PPI$aliasA %in% syn_DNV_genes & PPI$aliasB %in% syn_DNV_genes),])
...
iRef_syn = iRef_syn[which(!duplicated(iRef_syn[,c("aliasA", "aliasB")])),]

write.csv(iRef_cases, paste0(out_path, "/Cytoscape_DNV_network.csv"), 
...
degree_frame = data.frame(matrix(NA, nrow = net_perm, ncol = length(case_DNV_genes)))
names(degree_frame) = case_DNV_genes
...
for(i in c(1:net_perm)){
  ...
  disease_net = G0[which(G0$newA %in% case_DNV_genes | G0$newB %in% case_DNV_genes),]
  direct_disease_net = G0[which(G0$newA %in% case_DNV_genes & G0$newB %in% case_DNV_genes),]
...
write.csv(degree_frame, 
          file = paste0(base_path, "/intermediate/network_permutations/DNV_", today, ".csv"),
          quote = FALSE, row.names = FALSE)

# # # # # # # # # # CALCULATE P-VALUES # # # # # # # # # #
true_net_dens = nrow(PPI[which(PPI$aliasA %in% case_DNV_genes & PPI$aliasB %in% case_DNV_genes),])
...
for (j in c(1:ncol(degree_frame))){
  ...
  gene_dens[j] = nrow(gene_PPI[which(gene_PPI$aliasA %in% case_DNV_genes &
                                       gene_PPI$aliasB %in% case_DNV_genes),])
}

# Network permutation p-value
nGreater = length(perm_dens[which(perm_dens >= true_net_dens)])
pval = (nGreater/length(perm_dens))
write.table(paste0("Network connectivity p-value: ", pval), file = paste0(out_path, "/network_pval.txt"))

# Node p-values & node file
nodes = data.frame(matrix(NA, nrow = length(case_DNV_genes)), stringsAsFactors = FALSE)
...
# BH correction
nodes$pval[which(nodes$pval == 0)] <- (1/net_perm - .1/net_perm)
nodes$BH = p.adjust(nodes$pval, method = "BH")
nodes_adj = nodes[,c("Gene", "BH")]

write.csv(nodes_adj, file = paste0(out_path, "/Cytoscape_DNV_nodes.csv"), 
          row.names = FALSE, quote = FALSE)

# # # # # # SCRATCH # # # # # #

# Read in permutation results
#nodes_adj = read.csv(file = paste0(out_path, "/Cytoscape_DNV_nodes.csv"), stringsAsFactors = FALSE)
