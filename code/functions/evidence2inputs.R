...
run_artMS <- function(int_type, ev_file, key_file, quant_var){
  ...
  if(int_type != "NKX25"){
    artmsQuantification(yaml_config_file = paste0(base_path, "/input/evidence/expTFs/", int_type, "_config.yaml"))
  }
  ...
  artmsEvidenceToSaintExpress(evidence_file = ev_file, 
                              ...
                              output_file = paste0(base_path, "/input/evidence/expTFs/saintx_inputs/",int_type, "_", quant_var, ".txt"))
  
  artmsEvidenceToSAINTq(evidence_file = ev_file, 
                        keys_file = key_file, 
                        output_dir = paste0(base_path, "/input/evidence/expTFs/saintq_inputs/", int_type))
}