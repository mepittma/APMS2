...
#' @export
MaxQtoMSstatsAttempt <- function(evidence, 
                                annotation,
                                protGroups, 
                                proteinID="Proteins", 
  ...
  aggregateFun=summaryforMultipleRows
  experiment <- "DDA"
  
  ## evidence.txt file
  infile <- read.table(evidence, sep = "\t", header=T, stringsAsFactors = F)
  
  ## annotation.txt : Raw.file, Condition, BioReplicate, (IsotopeLabelType)
  annot <- read.table(annotation, sep = "\t", header=T, stringsAsFactors = F)
  ...
  proteinGroups <- read.table(protGroups, sep = "\t", header=T, stringsAsFactors = F)
...
.cast_maxquant_to_wide_glf <- function(d_long, aggregateFun=summaryforMultipleRows){
  d_long = as.data.table(d_long)
  print(typeof(d_long))
...
.cast_maxquant_to_wide_silac <- function(d_long, aggregateFun=summaryforMultipleRows){
...
.remove_feature_with_few <- function(df){
  df <- as.data.frame(df)
  y <- df[, -which(names(df) %in% c("Proteins", "Modified.sequence", "Charge"))]
  count_measure = rowSums(is.na(y))
  remove_feature_name <- df[which(count_measure < 3), c("Proteins", "Modified.sequence", "Charge")]
  
  df$Feature <- paste(df$Proteins, df$Modified.sequence, df$Charge, sep="_")
  ...
  df <- df[-which(df$Feature %in% remove_feature_name$Feature), ]
  df <- df[, -ncol(df)]
